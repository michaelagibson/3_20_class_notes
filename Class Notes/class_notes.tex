...
\begin{enumerate}
\item \un{system}: any collection of matter that can be uniquely identified and on which you can define macroscopic averages (not necessarily homogeneous)
\item \un{environment}: the complement of a system.  [environment] = [universe] - [system]
\item \un{extensive variables}: scales with the system size (i.e. volume, mass, number of particles, $e^-$, ...)
\begin{equation}
V = V_1 + V_2
\end{equation}
\item \un{intensive variables}: do not scale with system size (i.e. pressure, temperature, E-field, ...)
\item \un{``state" variables}: what you need to characterize a system (T, P, n, ...).  These are \bold{not} equal to the \bold{state} of a system.
\item \un{boundaries}: defined for a system.  These are very dependent on the topic and can be permeable (open to matter, changing n), closed (closed to matter), adiabatic (lack of heat flow), athermal (no heat flow), rigid (non-deformable), deformable, etc.  They can speak about extensive variables and limit/confine the system.
\end{enumerate}
...
\Delta E = \Delta U
\end{equation}
We assume that $U$, the internal energy, exists and it is a \un{state variable} that represents all types of energy exchange that can change the internal states of the system.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Lecture - September 5, 2014}
We have one form of energy transfer: \un{work}, which is equal to the force times the displacement.
\begin{equation}
W = \vec{F} \cdot d\vec{r}
\end{equation}
The formalism for work is $\partial W_i = y_i dx_i$, where $y_i$ is the force (intensive) and $dx_i$ is the response (extensive).  $(y_i, x_i)$ are a \bold{conjugate pair}.
\subsection{Two examples of work}
\bold{Deformation of a material}: $\partial W = v \bar{\bar{\sigma}} \cdot \bar{\bar{d\epsilon}}$ where $\sigma, \epsilon$ are tensors.  The shear \bold{stress} is denoted by off-diagonals $\sigma_{12}, \sigma_{13}, \sigma_{23}$.\\
...
\end{equation}
where $c_{ijkl}$ is the generalized elastic constant.  If \un{isotropic}, $\frac{\partial V}{\partial P}|_T = V \cdot \beta_T$ where $\beta_T$ is the isothermal compressibility.\\

\bold{Electrical work on a dielectric medium (isotropic)}: The energy stored in the dielectric is given by $\varepsilon = \vec{E}\cdot l$, and \boxed{\partial W = \varepsilon dq}.  Also, $q=D \cdot A$, where $D$, the electric displacement, is equal to $\varepsilon_0 E + \frac{P}{V}$ and $P$ is the total polarization.
\begin{align*}
...
\subsection{Practice}
\bold{Case 1 (Exact Differential):}
\begin{align*}
...
\bold{Case 2 (Inexact Differential):}  If we are missing a force, say $F_y'$, then the change is work would be
\begin{align*}
\partial W_\text{new} = \partial W_x + \partial W_z
\end{align*}
Consider two paths.  For each the amount of work done along the x-direction is not going to be the same.  However, since we are missing the y-component force the work for moving along each path is \un{different}, and path-dependent.  For path \#1 we might have $\int \partial W_x = \int F_x dx = 0$ whereas for path \#2 we could very well have $W_x = \int F_x dx \neq 0$.  If there are some \un{dissipative forces}, then we will also get path-dependence.
\begin{align*}
...
\subsection{Heat}
$Q =$ heat.  Adiabatic means $\partial Q = 0$.  The \un{first postulate of thermodynamics} is
\begin{equation}\boxed{
dU = \partial W + \partial Q
}\end{equation}
U is a property of a system - function of states.  $W$ and $Q$ are \un{not} (they refer to a transfer of energy).  Thompson (Rumford) measured mass change with varying temperature in water to help observe this phenomena.

\bold{Example:} Consider a simple system (only volume work) that evolves ``slowly'' through a series of equilibrium states (quasistatically).  Then $W = -\int P dV$.  An ideal gas has a pressure dependent on volume.  We have the following equation of state $P = \frac{nRT}{V}$ where work is
\begin{align*}
W &= -nRT\int \frac{dV}{V}\\
&= nRT \ln(\frac{V_f}{V_i})
\end{align*}
